#AutoIt3Wrapper_Icon=Icons\remoteWmiInfo.ico
#AutoIt3Wrapper_Res_Fileversion=0.7.10.2
...
#cs
	; $info mapping
	; PUT THIS SHIT IN A COOL AUTOIT-OBJECT SOON
	...
	; properties:
		OS
		current user (as array: 0-domain 1-name 2-def printer 3-???)
		default user (same, maybe printer?)
		IE
		Java
		LANDesk
		SCCM
	...
	; properties:
		...
		GUID
		Ping
		NICs (as 2D array? 0-name 1-IP 2-DHCP 3-MAC)
		Entity Location
		Wireless:
			SSID
			Signal
	...
	; monitor
	; properties (all arrays in case of multiple?):
		Model
		Serial
		PNP Name
		Availability
...
;~ 	14 noise
;~ 	15 channel
;~ 	16 DHCP (dynamic/static added to IP [11])
;~ 	17 ping
;~ 	18 BSSID
...
;~ if NOT StringInStr(FileGetAttrib($pathHistory), "H") Then FileSetAttrib($pathHistory, "+H")
...
Global $oRWIQuery = _AutoItObject_Class()
$oRWIQuery.Create()
$oRWIQuery.AddMethod("queryComputer", "oRWIQuery_queryComputer")
_AutoItObject_AddDestructor($oRWIQuery, "oRWIQuery_Destruct")

$oRWIQuery.AddProperty("
...
FileInstall("Icons\Gnome\Gnome-Document-Save.ico", "Icons\Gnome\Gnome-Document-Save.ico")
FileInstall("Icons\Gnome\Gnome-Preferences-Desktop-Remote-Desktop.ico", "Icons\Gnome\Gnome-Preferences-Desktop-Remote-Desktop.ico")
FileInstall("Icons\Gnome\Gnome-Application-Exit.ico", "Icons\Gnome\Gnome-Application-Exit.ico")
FileInstall("Icons\Gnome\Gnome-System-Search.ico", "Icons\Gnome\Gnome-System-Search.ico")
FileInstall("Icons\Gnome\Gnome-Emblem-Default.ico", "Icons\Gnome\Gnome-Emblem-Default.ico")
FileInstall("Icons\Gnome\Gnome-Edit-Undo.ico", "Icons\Gnome\Gnome-Edit-Undo.ico")
FileInstall("Icons\Gnome\Gnome-Edit-Clear.ico", "Icons\Gnome\Gnome-Edit-Clear.ico")
FileInstall("Icons\Gnome\Gnome-Video-Display.ico", "Icons\Gnome\Gnome-Video-Display.ico")
;~ FileInstall("Icons\Gnome\Gnome-Dialog-Question.ico", "Icons\Gnome\Gnome-Dialog-Question.ico")
...
_GUIImageList_AddIcon($hImage, "Icons\Gnome\Gnome-Network-Transmit-Receive.ico")
_GUIImageList_AddIcon($hImage, "Icons\Gnome\Gnome-Network-Offline.ico")
...
GUICtrlSetImage($btnGo, "Icons\Gnome\Gnome-Emblem-Default.ico", 0, 0)
...
GUICtrlSetImage($btnSave, "Icons\Gnome\Gnome-Document-Save.ico", 0, 0)
...
GUICtrlSetImage($btnRC, "Icons\Gnome\Gnome-Preferences-Desktop-Remote-Desktop.ico", 0, 0)
...
GUICtrlSetImage($btnLO, "Icons\Gnome\Gnome-Application-Exit.ico", 0, 0)
...
GUICtrlSetImage($btnDef, "Icons\Gnome\Gnome-Edit-Undo.ico", 0, 0)
...
;~ GUICtrlSetImage($btnAbout, "Icons\Gnome\Gnome-Dialog-Question.ico", 0, 0)
...
GUICtrlSetImage($btnShowEDID, "Icons\Gnome\Gnome-Video-Display.ico", 0, 0)
...
GUICtrlSetImage(-1, "Icons\Gnome\Gnome-Folder-Remote.ico", 1, 0)
...
GUICtrlSetImage(-1, "Icons\Gnome\Gnome-System-Software-Installer.ico", 1, 0)
...
GUICtrlSetImage($btnQBFill, "Icons\Gnome\Gnome-Insert-Text.ico", 1, 0)
...
GUICtrlSetImage($btnQBDefault, "Icons\Gnome\Gnome-View-Refresh.ico", 1, 0)
...
GUICtrlSetImage($btnLogClear, "Icons\Gnome\Gnome-Edit-Clear.ico", 1, 0)
GUICtrlCreateTabItem("")
...
$hProgress = GUICtrlCreateProgress(0, 0, -1, -1)

$statusBar = _GUICtrlStatusBar_Create($frmInfo)
; try to make a dynamically-sized status size
Dim $StatusBar1_PartsWidth[3] = [StringLen(_NowTime(5) & " >") * 6.5, 275, -1]
...
_GUICtrlStatusBar_EmbedControl($statusBar, 2, GUICtrlGetHandle($hProgress))
_GUICtrlStatusBar_SetMinHeight($statusBar, 20)

#cs
$hMenu = GUICtrlCreateContextMenu()
_GUICtrlCreateODMenuItem("test1", $hMenu, "Icons\Gnome\Gnome-Computer.ico")
_GUICtrlCreateODMenuItem("test2", $hMenu, "Icons\Gnome\Gnome-Document-Open.ico")
		...
		Case $btnGo
			...
			if StringLeft($sComp, 5) = "call " Then
				if $debug AND @Compiled Then
					...
					Call($sFunc, $aArgs)
				EndIf
			ElseIf _netValidIP($sComp) Then
				$sHost = _TCPIpToName($sComp)
				if @error Then
					appendLog("Error " & @error & " getting hostname from IP")
					upStatus("Couldn't get hostname from IP")
					ContinueLoop
				Else
					$sComp = $sHost
				EndIf
			ElseIf NOT _computerNameLegal($sComp) Then
				appendLog("Name contains illegal characters")
				upStatus("Name contains illegal characters")
				ContinueLoop
			EndIf
...
;~ 	_filePush($pathHistory, $compName)
	...
	Global $oRWI = $oRWIQuery.Create
	$oRWI.QueryComputer($compName, $verbose)
	...
	; defaultuser - $info[7]
	If $info[7] <> "" Then
		if $info[7] = $info[6] Then
			$aDUserInfo = $aUserInfo
		Else
...
Func oRWIQuery_queryComputer($oSelf, $compName, $verbose)
	Local $iProgressParts = 7
	...
	GUICtrlSetData($hProgress, (1/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (2/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (3/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (4/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (5/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (6/$iProgressParts)*100)
	...
	GUICtrlSetData($hProgress, (7/$iProgressParts)*100)
